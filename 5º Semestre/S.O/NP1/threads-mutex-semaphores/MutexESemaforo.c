...
pthread_mutex_t count_mutex = PTHREAD_MUTEX_INITIALIZER; //tipo mutex

#define N 2 // number of threads

sem_t s; //tipo semaforo

int x = 0; //região crítica (uma por vez vai mexer nela)

int main(void) {
    pthread_t tids[N];
    int i = 0;

    sem_init(&s, 0, 1); // inicializa o semáforo com valor 1, o 0 ali significa que estou usando ele pra threads
    ...
    * terminate */
    for(i=0; i<N; i++) {
        pthread_join(tids[i], NULL); //espera ate terminar
...
void *mythread(void *data) {
    ...
    printf("**MUTEX**\n");
    while(x < 10) {
        pthread_mutex_lock(&count_mutex); //operação down
        ...
        * another thread gets the chance to run. */
        ...
        x++;
        pthread_mutex_unlock(&count_mutex);//operação up
        sleep(1);
    }
