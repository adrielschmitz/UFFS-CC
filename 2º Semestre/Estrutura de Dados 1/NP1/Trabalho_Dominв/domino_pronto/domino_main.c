...
int main(){
	...
	Asciiart *desenho_jogador, *desenho_mesa;
	int i, cont = 0, cont2 = 0, escolha_lado, numero_pecas, aux_aux_tamanhoanho, random, aux_tamanho;
	monte = jogador = aux = bot = mesa = pecajogada = inicializa();
	desenho_jogador = desenho_mesa = inicializa_ascii_art();
	srand(clock());
 ...
 //BLOCO DA VEZ DO JOGADOR---------------------------------------------------------------------------------------------------------
	...
	do{
		while(flag_jogador_ativo){
			...
			printf("MESA\n");
			desenho_mesa = chama_desenho(mesa);
			printar_ascii_art_mesa(desenho_mesa);
			printf("\nMÃO DO JOGADOR\n");
			desenho_jogador = chama_desenho(jogador);
			printar_ascii_art_jogador(desenho_jogador);
		 	int opcao_peca_escolhida;
			numero_pecas = contar_elementos(jogador);
			printf("Informe a peça que deseja jogar ou [0] para comprar: ");
			...
			if(opcao_peca_escolhida >= 1 && opcao_peca_escolhida <= numero_pecas && numero_pecas != 0){
				...
				if(opcao_peca_escolhida == 1){
					...
					while(flag_escolhe_lado){
						printf("Escolha o lado da mesa que deseja inserir a peça.\nEsquerdo[1] | Direito[2]: ");
						...
						if(escolha_lado == 1){
							...
							printf("Opção inválida. Escolha somente 1 ou 2.\n");						
						}									
					}
				}else{
					if(testa_pecas(jogador, mesa) == 1){
						...
						printf("\nVocê não possui mais peças válidas. Digite [0] para comprar no monte.\n");
						sleep(3);
					}else{
						system("clear");
				...
				if(contar_elementos(monte) > 0){
					...
					do{
						...
						while(pesca_escolha != 0 && pesca_escolha != 1){
							printf("Opção inválida!\nPrecione [0] para pescar | Para cancelar [1]\n");
							while(scanf("%d", &pesca_escolha) != 1) getchar();
						}
						if(pesca_escolha == 1){
							break;
						}
						aux_aux_tamanhoanho = contar_elementos(monte);
						random = abs(rand()%(aux_aux_tamanhoanho));
					...
					system("clear");
					printf("0 PEÇAS NO MONTE!.\n");
					sleep(2);
				}		
			}else{
				system("clear");
				printf("\tOpção nao válida. Insira um índice entre 1 a [%d].\n", numero_pecas);
				sleep(2);
			}			
		}
		if(jogador == NULL){ // caso o jogador nao estiver mais peças na mao, o bot nao precisa jogar mais alguma peça, fim do jogo;
			break;
		}
